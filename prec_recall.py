...
def prec_recall_kmeans():
    for i in list_of_files:
        ...
        for m in range(15):
            ...
            list_tmp = np.array(
                ground_truth.loc[ground_truth['cluster_id'] == m]['method_name'])
        ...
        list_tmp = np.array(
            ground_truth.loc[ground_truth['cluster_id'] == 'None']['method_name'])
        for m in range(len(list_tmp)):
            for n in list_tmp[m+1:]:
                list_of_files_ground_truth.add((list_tmp[m], n))

        string_for_writing = ''
        precision = len(intrapair_kmeans.intersection(
            list_of_files_ground_truth))/len(intrapair_kmeans)
        name_class = i.replace(".csv", "")
        string_for_writing += f"\nPrecision for { name_class } is : {precision}\n"

        recall = len(intrapair_kmeans.intersection(
            list_of_files_ground_truth))/len(list_of_files_ground_truth)
