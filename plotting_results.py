...
for name in god_classes:
    ...
    g = random.random()
    color = (r, g, b)
    ax[count].plot(df['k'], df['score'], color=color)

    ax[count].set_xlabel('K')
    ax[count].set_ylabel('Sihlouette Score')
    x, y = df['score'].idxmax(axis=0)+2, max(df['score'])
    ax[count].text(x, y, '%.2f' % y, ha='left', va='top')
    ax[count].scatter(x, y, color='black', s=20)
    count += 1
